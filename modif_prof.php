...
$id=$_GET['modif_prof'];
$ligne=mysqli_fetch_array(mysqli_query($link,"select * from prof where numprof='$id'"));
...
if(isset($_POST['nom'])){
	if($_POST['adresse']!="" and $_POST['phone']!=""){
		...
		mysqli_query($link,"update prof set adresse='$adresse', telephone='$phone' where numprof='$id'");
...
$id=$_GET['supp_prof'];
mysqli_query($link,"delete from prof where numprof='$id'");
