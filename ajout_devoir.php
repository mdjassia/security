...
$donnee=mysqli_query($link,"select codemat,nommat from matiere,classe where matiere.codecl=classe.codecl and nom='$nomcl' and promotion='$promo'");
?>
<FIELDSET>
 ...
 */
$data=mysqli_query($link,"select count(*) as nb from devoir where codecl=(select codecl from classe where nom='$nomcl' and promotion='$promo') and codemat='$codemat' and numsem='$semestre' and n_devoir='$n_devoir'");
/*
 pour verifier si l'enseignemet (codecl,nommat,numsem) existe ou  pas
 */
$valider=mysqli_query($link,"select count(*) as nb from enseignement where codecl=(select codecl from classe where nom='$nomcl' and promotion='$promo') and codemat='$codemat' and numsem='$semestre'");
...
$bool=true;
	...
	*/
	...
	if($bool==true){
	$codeclasse=mysqli_query($link,"select codecl from classe where nom='$nomcl' and promotion='$promo'");
	$code=mysqli_fetch_array($codeclasse);
	$codecl=$code['codecl'];
	mysqli_query($link,"insert into devoir(date_dev,coeficient,codemat,codecl,numsem,n_devoir) values('$date','$coefficient','$codemat','$codecl','$semestre','$n_devoir')");
