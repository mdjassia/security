...
$link = mysqli_connect("localhost", "root", "sasasasasa");
...
if(isset($_POST['nom'])){
	...
	$pseudo=$_POST['pseudo'];
	$passe=$_POST['mdp'];
	$nb=mysqli_fetch_array(mysqli_query($link,"select count(*) as nb from eleve where nomel='$nom' and prenomel='$prenom'"));
	if($nb['nb']!=0){
	?><SCRIPT LANGUAGE="Javascript">alert("erreur! cet enregistrement existe d�ja!");</SCRIPT><?php
	}
	else{
	$data=mysqli_fetch_array(mysqli_query($link,"select codecl from classe where nom='$nomcl' and promotion='$promo'"));
	$codecl=$data['codecl'];
	mysqli_query($link,"insert into eleve(nomel,prenomel,date_naissance,adresse,telephone,codecl) values('$nom','$prenom','$date','$adresse','$phone','$codecl')");
	/*		Ajouter le num dans le login    */
	$numel=mysqli_fetch_array(mysqli_query($link,"select numel from eleve where nomel='$nom' and prenomel='$prenom'"));
	$num=$numel['numel'];
	mysqli_query($link,"insert into login(Num,pseudo,passe,type) values('$num','$pseudo','$passe','etudiant')");
...
<?php 
$retour=mysqli_query($link,"select distinct nom from classe"); // afficher les classes
