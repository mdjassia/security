...
$id=$_GET['modif_dev'];
$ligne=mysqli_fetch_array(mysqli_query($link,"select * from classe,devoir,matiere where classe.codecl=devoir.codecl and matiere.codemat=devoir.codemat and numdev='$id'"));//Pour afficher le nom de l'eleve et la note par deflault et le devoir,
$date=$ligne['date_dev'];

?>
<center><pre><h1>Modifier un devoir</h1>
<form action="modif_devoir.php" method="POST" class="formulaire">
Mati�re :<?php echo $ligne['nommat']; ?><br/>
...
if(isset($_POST['n_devoir'])){//s'il a cliquer sur le bouton modifier
	...
	if(($_POST['n_devoir']=="1" or $_POST['n_devoir']=="2") and $_POST['date']!="" and $_POST['coeficient']!=""){
		...
		$date=$_POST['date'];
		$compte=mysqli_fetch_array(mysqli_query($link,"select count(*) as nb from devoir where n_devoir='$n_devoir' and numdev='$numdev' and date_dev='$date'"));
		if($compte['nb']!=0){//deux devoir similaire()2 devoirs par matiere
		?> <SCRIPT LANGUAGE="Javascript">	alert("erreur de modification,ce devoir existe d�ja(verifier le numero de devoir)"); </SCRIPT> <?php
		}
		else{
			mysqli_query($link,"update devoir set n_devoir='$n_devoir', coeficient='$coeficient',date_dev='$date' where numdev='$id'");
...
$id=$_GET['supp_dev'];
mysqli_query($link,"delete from devoir where numdev='$id'");
mysqli_query($link,"delete from evaluation where numdev='$id'");
?> <SCRIPT LANGUAGE="Javascript">	alert("Supprim� avec succ�s!\ntous les evaluations de ce devoir ont �t� supprim�es"); </SCRIPT> <?php
echo '<br/><br/><a href="afficher_devoir.php">Revenir � la page � l\'affichage</a>'; //on revient � la page princippale car on n'a plus l'id dont on affiche la matiere dans la modification
}
?>
</center></pre>
</div>