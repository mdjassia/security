...
$donnee=mysqli_query($link,"select codemat,nommat from matiere,classe where matiere.codecl=classe.codecl and classe.nom='$nomcl' and promotion='$promo'");
$prof=mysqli_query($link,"select numprof,nom,prenom from prof");
?>
...
$codeclasse=mysqli_fetch_array(mysqli_query($link,"select codecl from classe where nom='$nomcl' and promotion='$promo'")) ;
$codecl=$codeclasse['codecl'];
/*
 pour ne pas ajouter deux enseignements similaires
 */
$data=mysqli_query($link,"select count(*) as nb from enseignement where codecl='$codecl'  and codemat='$codemat' and numsem='$semestre'");
/*
...
$bool=true;
	...
	*/
	...
	if($bool==true){
	mysqli_query($link,"insert into enseignement(codecl,codemat,numprof,numsem) values('$codecl','$codemat','$n_prof','$semestre')");
...
$donnee=mysqli_query($link,"select distinct nom from classe"); 
?>
 <form action="ajout_enseignement.php" method="POST" class="formulaire">
