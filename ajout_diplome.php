...
$nomclasse=mysqli_query($link,"select distinct nom from classe");
 ?>
 <img src="titre_img/ajout_diplome.png" class="position_titre">
 <form action="ajout_diplome.php" method="POST" class="formulaire">
...
$data=mysqli_query($link,"select numel,nomel,prenomel from eleve where codecl=(select codecl from classe where nom='$nomcl' and promotion='$promo')");
$titre=mysqli_query($link,"select numdip,titre_dip from diplome");
?>
<img src="titre_img/ajout_diplome.png" class="position_titre">
 <form action="ajout_diplome.php" method="POST" class="formulaire">
...
if($_POST['note']!="" and $_POST['lieu']!="" and $_POST['comment']!="" and $_POST['etabli']!="" and $_POST['ann_obt']!=""){
	...
	$numdip=$_POST['titre'];
	echo 'numel--> '.$numel;
	$nb=mysqli_fetch_array(mysqli_query($link,"select count(*) as nb from eleve_diplome where numel='$numel'"));
		if($nb['nb']!=0){
			?><SCRIPT LANGUAGE="Javascript">alert("erreur! cet enregistrement existe d�ja!");</SCRIPT><?php
		}
		else{
		mysqli_query($link,"insert into eleve_diplome(numdip,numel,note,commentaire,etablissement,lieu,annee_obtention) values('$numdip','$numel','$note','$comment','$etabli','$lieu','$annee')");
...
echo '<br/><br/><a href="ajout_diplome.php?ajout_diplome">Revenir � la page pr�c�dente !</a>';
	...
	$nb=mysqli_fetch_array(mysqli_query($link,"select count(*) as nb from diplome where titre_dip='$titre'"));
	if($nb['nb']!=0){
	?><SCRIPT LANGUAGE="Javascript">alert("erreur! cet enregistrement existe d�ja!");</SCRIPT><?php
	}
	else{
	mysqli_query($link,"insert into diplome(titre_dip) values('$titre')");
	?>	<SCRIPT LANGUAGE="Javascript">alert("Ajout avec succ�s!");</SCRIPT> 	<?php
	}
