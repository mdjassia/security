...
<?php }
	...
	if($_POST['nommat']!=""){
		...
		$compte=mysqli_fetch_array(mysqli_query($link,"select count(*) as nb from matiere where nommat='$nommat' and codecl='$codecl'"));
		$bool=true;
		if($compte['nb']>0){
			$bool=false;
		...
		if($bool==true){
			mysqli_query($link,"insert into matiere(nommat,codecl) values ('$nommat','$codecl')");
