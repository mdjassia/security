...
$data=mysqli_query($link,"select promotion from classe where nom='$nomcl' order by promotion desc");
?>
...
<br/><br/><a href="index.php?">Revenir � la page pr�c�dente !</a>
<?php } 
if(isset($_POST['promotion'])){
$nomcl=$_SESSION['nomcl'];
$promo=$_POST['promotion'];
$donnee=mysqli_query($link,"select numel,nomel,prenomel,date_naissance,adresse,telephone,eleve.codecl,nom,promotion from eleve,classe where eleve.codecl=classe.codecl and nom='$nomcl' and promotion='$promo'");
?>
...
echo '<td><a href="modif_eleve.php?modif_el='.$a['numel'].'">modifier</a></td><td><a href="modif_eleve.php?supp_el='.$a['numel'].'" onclick="return(confirm(\'Etes-vous s�r de vouloir supprimer cette entr�e?\ntous les enregistrements en relation avec cette entr�e seront perdus\'));">supprimer</a></td>';}
...
<?php
echo '<br/><br/><a href="listeEtudiant.php?nomcl='.$nomcl.'">Revenir � la page pr�c�dente !</a>';
}
if(isset($_GET['voir_ensei'])){
$id=$_GET['voir_ensei'];
$data=mysqli_query($link,"select prof.nom,prenom,nomel,prenomel,classe.nom as nomcl,numsem,nommat,prof.adresse,promotion from prof,matiere,classe,eleve,enseignement where prof.numprof=enseignement.numprof and enseignement.codemat=matiere.codemat and eleve.codecl=classe.codecl and classe.codecl=enseignement.codecl and numel='$id'");
?>
<h2>Les enseignants de l'�tudiant choisis : </h2><br/>
<center><table id="rounded-corner">
