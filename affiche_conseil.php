...
$retour=mysqli_query($link,"select distinct nom from classe"); //pour afficher les classe existantes
?>
<div class="corp">
...
mysqli_query($link,"delete from conseil where id='$id'");
...
$numsem=$_POST['numsem'];
$donnee=mysqli_query($link,"select * from classe,conseil where classe.codecl=conseil.codecl and classe.codecl=(select codecl from classe where nom='$nomcl' and promotion='$promo') and numsem='$numsem'");//select nommat from matiere,classe where matiere.codecl=classe.codecl and classe.nom='$classe'
