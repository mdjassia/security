...
$id=$_GET['modif_eval'];
$ligne=mysqli_fetch_array(mysqli_query($link,"select * from evaluation,eleve,classe where eleve.numel=evaluation.numel and eleve.codecl=classe.codecl and numeval='$id'"));//Pour afficher le nom de l'eleve et la note par deflault et le devoir,
$codecl=$ligne['codecl'];
$eleve=mysqli_query($link,"select numel,nomel,prenomel from eleve where codecl='$codecl'");
$numdev=stripslashes($ligne['numdev']);
$mat_dev=mysqli_fetch_array(mysqli_query($link,"select * from matiere,devoir where devoir.codemat=matiere.codemat and numdev='$numdev'"));//pour selection la classe par defualt et afficher la promotion
?>
...
if(isset($_POST['numel'])){//s'il a cliquer sur le bouton modifier
	if($_POST['note']!=""){
		...
		mysqli_query($link,"update evaluation set numel='$numel', note='$note' where numeval='$id'");
...
$id=$_GET['supp_eval'];
mysqli_query($link,"delete from evaluation where numeval='$id'");
