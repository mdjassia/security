...
$donnee=mysqli_query("select numdev,date_dev,nommat,nom,coeficient,numsem,n_devoir from devoir,matiere,classe where matiere.codemat=devoir.codemat and classe.codecl=devoir.codecl and classe.nom='$nomcl' and devoir.numsem='$semestre' and matiere.nommat='$nommat' and promotion='$promo'");
...
$compte=mysqli_fetch_array(mysqli_query("select count(*) as nb from evaluation where numdev='$numdev' and numel='$numel'"));//pour ne pas repeter le meme enregistrement
if($compte['nb']>0){
?>
...
mysqli_query("insert into evaluation(numdev,numel,note) values('$numdev','$numel','$note')");
?>
<SCRIPT LANGUAGE="Javascript">
...
$donnee=mysqli_fetch_array(mysqli_query($link,"select date_dev,coeficient,n_devoir from devoir where numdev='$numdev'"));//  pour afficher les information du devoir k'il a choisis pour ajouter un devoir
$data=mysqli_query($link,"select numel,nomel,prenomel from eleve where codecl=(select codecl from classe where nom='$nomcl' and promotion='$promo')");//pour afficher les etudiants
?>
<form method="POST" action="ajout_eval.php" class="formulaire">
...
<?php while($a=mysqi_fetch_array($data)){
...
$data=mysqli_query($link,"select distinct promotion from classe order by promotion desc");
?>
...
<?php
$data=mysqli_query($link,"select distinct nom from classe"); ?>
